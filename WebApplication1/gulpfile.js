...
const style = e => {
    ...
    for (let key in path) {
        ...
        gulp.src(path[key])
            ...
            .pipe(gulp.dest(config.outPath.css))
            .pipe(notify({
                message: `${path[key][0]} -> ${path[key][0].replace('src', 'content').replace(/scss/, 'css2017').replace(/scss/, 'css')}`
            }));
...
gulp.task('style', () => {
    ...
    for (let key in path) {
        ...
        gulp.src(path[key])
            ...
            .pipe(gulp.dest(config.outPath.css))
            .pipe(notify({
                message: `src/sass/${key}.scss -> Content/css/${key}.min.css`
            }));
...
const copy = e => {
    ...
    gulp.src(changedPath)
        ...
        .pipe(notify({
            message: `${changedPath} -> ${changedPath.replace('src', 'script')}`
        }));
